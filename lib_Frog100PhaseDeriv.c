...
double Interaction(double theta_passive, double theta_active, int passive, int active){
	...
	double distance = sqrt(pow(x_passive - x_active, 2) + pow(y_passive - y_active, 2)) * 1.000000;
...
double f_0(double t, double *x){
	...
	for(fi = 0; fi < 100; fi++){
		if(fi != IndividualID){
			...
			x_active = fi % 10;
			y_active = fi / 10;
			distance = sqrt(pow(x_passive - x_active, 2) + pow(y_passive - y_active, 2)) * 1.000000;
