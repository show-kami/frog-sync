...
execute <- function(){
	for(i in 1:num.timeslice){
		...
		for(j in 1:num.frogs){
			phase_j <- data[i, 1+j]
			par(new = T)
			plot(cos(phase_j), sin(phase_j) , pch = 16, cex = 3, col = "black", xlim = c(-1,1), ylim = c(-1,1) , xlab = "", ylab = "")
		}
		par(new = T)
		plot(OrderParaX, OrderParaY, pch = 16, cex = 3, col = "gold", xlim = c(-1,1), ylim = c(-1,1))
